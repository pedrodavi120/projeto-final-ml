...
// --- IMPORTANTE: GARANTA QUE ESTAS IMAGENS EXISTAM NA PASTA ASSETS ---
// Se não tiver a logo, comente a linha abaixo e o uso da imagem no Header
import logoSentinela from './assets/logosentinela.png';

// Tenta importar o JSON do modelo. Se falhar (arquivo não existe), usa undefined.
let triageConfigRaw;
try {
  triageConfigRaw = await import('./assets/model_triage_config.json');
} catch (e) {
  console.warn("Arquivo de configuração do modelo não encontrado. Usando valores padrão.");
}
const triageConfig = triageConfigRaw || {
    "thresholds": {
        "spo2_critical": 92,
        "bpm_tachycardia": 110,
        "bpm_bradycardia": 50,
      ...
      <aside className="w-full md:w-72 bg-white border-r border-slate-200 flex-shrink-0 flex flex-col justify-between z-20 shadow-xl">
        <div>
          ...
          <div className="p-8 flex justify-center items-center bg-gradient-to-r from-teal-600 to-teal-700 text-white shadow-lg min-h-[120px]">
            {/* TENTE CARREGAR A LOGO, SE FALHAR MOSTRA TEXTO */}
            <img 
              src={logoSentinela} 
              alt="Sentinela" 
              className="w-auto h-24 object-contain drop-shadow-md hover:scale-105 transition-transform" 
              onError={(e) => {e.target.style.display='none'; e.target.nextSibling.style.display='block'}}
            />
            <div className="hidden text-center">
                <h1 className="text-2xl font-black tracking-tighter">SENTINELA</h1>
                <p className="text-[10px] uppercase tracking-widest opacity-80">Health AI</p>
            </div>
          </div>
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
             ...
             <div>
                <p className="text-xs font-bold text-slate-700">Pedro Davi</p>
                <p className="text-[10px] text-slate-500 font-medium">Aluno TADS • Online</p>
             </div>
          </div>
        </div>
      </aside>
...
function TriageView() {
  ...
  // MOTOR DE INFERÊNCIA CLIENT-SIDE (ATUALIZADO PARA USAR JSON)
  const calculateRisk = () => {
    setProcessing(true);
    setPrediction(null);
    
    // Extrai configurações aprendidas no Python
    const { thresholds, risk_weights } = triageConfig;
    ...
    setTimeout(() => {
      ...
      // Usa threshold dinâmico do JSON
      if (vitals.spo2 <= thresholds.spo2_critical) {
         riskScore += risk_weights.respiratory; 
         reasons.push(`Hipóxia (<${thresholds.spo2_critical}%)`);
      } else if (vitals.spo2 <= 95) {
         riskScore += 10;
      }

      // --- CAMADA 2: AVALIAÇÃO HEMODINÂMICA ---
      if (vitals.bpm > thresholds.bpm_tachycardia || vitals.bpm < thresholds.bpm_bradycardia) {
         riskScore += 25;
         reasons.push(`Freq. Cardíaca Anormal`);
      }
      
      if (vitals.pressure < thresholds.sys_hypotension) {
         riskScore += risk_weights.hemodynamic;
         reasons.push('Hipotensão Sistólica');
      }

      // --- CAMADA 3: SINAIS SISTÊMICOS (SIRS) ---
      if (vitals.temp > thresholds.temp_fever || vitals.temp < 36) {
         riskScore += risk_weights.systemic;
         // Cruzamento de dados (Feature Interaction do Gradient Boosting)
         if (vitals.bpm > 90) {
            riskScore += 30; 
      ...
      const confidence = isCritical 
        ? Math.min(60 + riskScore, 99) 
        : Math.max(99 - riskScore, 60);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                 ...
                 <div className="space-y-4">
                    <div className="flex justify-between">
                       ...
                       <span className={`text-xs font-bold px-2 py-0.5 rounded ${vitals.spo2 < triageConfig.thresholds.spo2_critical ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>{vitals.spo2}%</span>
                    </div>
                 ...
                 <div className="space-y-4">
                    <div className="flex justify-between">
                       ...
                       <span className={`text-xs font-bold px-2 py-0.5 rounded ${vitals.bpm > triageConfig.thresholds.bpm_tachycardia ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-600'}`}>{vitals.bpm} bpm</span>
                    </div>
                 ...
                 <div className="space-y-4">
                    <div className="flex justify-between">
                       ...
                       <span className={`text-xs font-bold px-2 py-0.5 rounded ${vitals.temp > triageConfig.thresholds.temp_fever ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>{vitals.temp}°C</span>
                    </div>
                 ...
                 <div className="space-y-4">
                    <div className="flex justify-between">
                       ...
                       <span className={`text-xs font-bold px-2 py-0.5 rounded ${vitals.pressure < triageConfig.thresholds.sys_hypotension ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>{vitals.pressure} mmHg</span>
                    </div>
